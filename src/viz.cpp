...
void draw_hex(cairo_t *cr, std::string shape = "00-000", int ix=0, int iy=0){
    ...
    for (int i = 1; i <= 6; ++i){
      if (shape[i-1] == '-'){
        ...
        x = c_x + side * std::cos( (60*i-120)* M_PI / 180.0) * 0.6*edge;
        y = c_y + side * std::sin( (60*i-120)* M_PI / 180.0) * 0.6*edge;
        cairo_line_to(cr, x, y);
...
//        std::cout << x << y << "\n";
        ...
        x = c_x + side * std::cos( (60*i-120)* M_PI / 180.0) * 1.4*edge;
        y = c_y + side * std::sin( (60*i-120)* M_PI / 180.0) * 1.4*edge;
        cairo_line_to(cr, x, y);
...
void draw_hex_fill(cairo_t *cr, std::string shape = "00-000", int ix=0, int iy=0, std::vector<std::string> v = {}){
    ...
    //if (parity(shape) > 1){
    if (iso_to(v, shape) == 0){
      //cairo_set_source_rgb (cr, 1, 0.7, 0.9);
      cairo_set_source_rgb (cr, 1, 0, 0);
      //cairo_set_source_rgb(cr, 0, 0, 0);
    //} else if (parity(shape) > 0){
    } else if (iso_to(v, shape) == 1){
      cairo_set_source_rgb (cr, 0.3, 0.3, 1);
      //cairo_set_source_rgb(cr, 0, 0, 0);
    } else {
      cairo_set_source_rgb (cr, 1, 1, 0);
      //cairo_set_source_rgb (cr, 0.7, 0, 0.9);
    }
    cairo_fill( cr );
}
